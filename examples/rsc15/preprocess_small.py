...
# remove sessions of length < 5
session_lengths = data.groupby('SessionId').size()
data = data[np.in1d(data.SessionId, session_lengths[session_lengths>=5].index)]
...
# after deleting items, remove again sessions of length < 5
data = data[np.in1d(data.SessionId, session_lengths[session_lengths>=5].index)]

# set last day for validation, day before the last day for training
tmax = data.Time.max()
session_max_times = data.groupby('SessionId').Time.max()
session_train = session_max_times[(session_max_times < tmax-timedelta(days=1)) & (session_max_times >= tmax-timedelta(days=2))].index
...
# after deleting items in valid, remove again valid sessions of length < 5
tslength = valid.groupby('SessionId').size()
valid = valid[np.in1d(valid.SessionId, tslength[tslength >= 5].index)]
